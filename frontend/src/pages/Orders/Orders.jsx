  ...
  if (status === 'cancelled') {
    return (
      <div style={{
        ...
        display: 'flex', alignItems: 'center', gap: '8px', margin: '14px 0 4px',
          ...
          return (
            <div key={step.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', flex: 1 }}>
              <div style={{
                ...
                background: done ? '#FF6B00' : '#F5F5F5',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '15px',
                boxShadow: active ? '0 0 0 5px rgba(255,107,0,0.15)' : 'none',
                transition: 'all 0.3s ease',
              }}>
                {done
                  ? <span>{step.emoji}</span>
                  : <span style={{ fontSize: '11px', fontWeight: '700', color: '#CCC' }}>{i + 1}</span>
                }
              </div>
              <span style={{
                fontSize: '9px', fontWeight: active ? '800' : '500',
                color: active ? '#FF6B00' : done ? '#888' : '#CCC',
...
function SkeletonCard() {
  return (
    <div style={{
      ...
      {[80, 120, 50].map((w, i) => (
        <div key={i} style={{
          height: i === 1 ? '14px' : '12px',
          width: `${w}%`, maxWidth: `${w * 2.8}px`,
...
export default function Orders() {
  ...
  return (
    <div style={{
      background: '#F7F7F8', minHeight: '100vh', paddingBottom: '90px',
      direction: isRTL ? 'rtl' : 'ltr', fontFamily: "'Plus Jakarta Sans', sans-serif",
    }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
          <div>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '12px', fontWeight: '600', letterSpacing: '0.05em', marginBottom: '2px' }}>
              SUIVI
            </p>
            <h1 style={{ color: '#fff', fontSize: '22px', fontWeight: '900', letterSpacing: '-0.02em' }}>
              Mes Commandes
            </h1>
          </div>
            ...
            >
              ...
              {count > 0 && (
                <span style={{
                  ...
                  background: tab === key ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.12)',
        ...
        {loading ? (
          ...
          >
            ...
            <button onClick={() => navigate('/')} style={{
              background: '#FF6B00', color: '#fff', padding: '13px 32px',
              borderRadius: '14px', fontWeight: '800', border: 'none', cursor: 'pointer',
              fontSize: '14px', boxShadow: '0 6px 20px rgba(255,107,0,0.35)',
              fontFamily: 'inherit',
            }}>
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <div style={{
                      height: '3px',
                      background: 'linear-gradient(90deg, #FF6B00, #FF9A00, #FFC107)',
                    }} />
                  )}
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '6px', flexWrap: 'wrap' }}>
                          ...
                          {isActive && (
                            <span style={{
                              ...
                              fontSize: '10px', fontWeight: '700', color: '#FF6B00',
                            }}>
                        ...
                        {/* Restaurant name */}
                        <p style={{
                          fontSize: '16px', fontWeight: '800', color: '#111',
                        ...
                        <p style={{ fontSize: '11px', color: '#BBB', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '4px' }}>
                          ...
                          {order.items?.length > 0 && (
                            <span style={{ marginLeft: '4px' }}>· {order.items.length} article{order.items.length > 1 ? 's' : ''}</span>
                          )}
                        </p>
                      </div>
                    ...
                    {/* Timeline always visible for active */}
                    ...
                    {isOpen && (
                      ...
                      >
                        <div style={{ padding: '0 16px 16px', borderTop: '1px solid #F5F5F5' }}>
                          ...
                          {/* Timeline for completed/cancelled */}
                          {!isActive && <OrderTimeline status={order.status} />}

                          {/* Delivery address */}
                          <div style={{
                            background: '#F9F9F9', borderRadius: '12px',
                            padding: '11px 13px', marginBottom: '12px',
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              paddingTop: '8px', borderTop: '1px solid #EBEBEB',
                          ...
                          {order.status === 'delivered' && (
                            <div style={{ display: 'flex', gap: '10px' }}>
                              ...
                              <button
                                onClick={() => toast('Notation à venir ⭐')}
                                style={{
                                  width: '46px', height: '46px', borderRadius: '14px',
                                  background: '#FFF3E8', color: '#FF6B00',
                                  border: '1.5px solid #FFE0C0', cursor: 'pointer',
                                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                                  flexShrink: 0,
                                }}
                              >
